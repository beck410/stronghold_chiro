...
class Tribe__Events__Tickets__Eventbrite__API {
	...
	public function sync_venue( $post ) {
		...
		if ( $country ) {
			...
			// $args['venue.address.country_name'] = tribe_get_country( $venue->ID );
		...
		if ( class_exists( 'Tribe__Utils__Coordinates_Provider' ) && ( empty( $args['venue.address.latitude'] ) || empty( $args['venue.address.longitude'] ) ) ) {
			...
			if ( empty( $coordinates ) ) {
				...
				Tribe__Events__Tickets__Eventbrite__Main::instance()->throw_notice( $venue, $message_error );
				return false;
			}
	...
	public function sync_event( $post, $params = array() ) {
		...
		if ( ! isset( $args['event.venue_id'] ) || false === $args['event.venue_id'] ) {
			if ( 'create' === $mode ) {
				Tribe__Events__Tickets__Eventbrite__Main::instance()->throw_notice( $post, __( 'Eventbrite requires a Venue and it must have a valid Address.', 'tribe-eventbrite' ) );
				return false;
			}
